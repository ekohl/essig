...
microcontroller:^(IDENTIFIER ^(PARAMETERS parameter*) ^(REGISTERS register*) ^(INSTRUCTIONS instruction*))
	;
...
instruction:	^(IDENTIFIER param* arg* expr+)
	;

param:		(SIZE | CLOCK) NUMBER
	|	^(OP_CODE (NUMBER | (IDENTIFIER (LBRACE NUMBER)?))*)
	;

arg:		IDENTIFIER
	;

expr	:	^(ASSIGN IDENTIFIER word (operator word)*)
	|	^(IF condition expr+ (ELSE expr+)?)
	;


word	:	NOT? (IDENTIFIER word? | NUMBER)
	;

condition:	(word (operator word)?) EQUALS word
	;
